...
function getSystemPrompt() {
  ...
  return "Tum ek helpful AI assistant ho. Hamesha Roman Urdu me jawab do, simple aur friendly andaz me.";
}
...
function saveSession() {
  ...
  const firstContent = Array.isArray(userMsgs[0].content)
    ? (userMsgs[0].content.find(p => p.type === "text")?.text || "Image")
    : userMsgs[0].content;
  const title = firstContent.substring(0, 30) + (firstContent.length > 30 ? "..." : "");
...
function loadSession(id) {
  ...
  messages.filter(m => m.role !== "system").forEach(m => {
    if (Array.isArray(m.content)) {
      ...
      appendMessage(m.role, txtPart?.text || "", false, imgPart?.image_url?.url || null);
    } else {
...
(function initParticles() {
  ...
  function randomParticle() {
    return { x: Math.random()*W, y: Math.random()*H, r: Math.random()*1.8+0.4,
      dx: (Math.random()-0.5)*0.4, dy: (Math.random()-0.5)*0.4, alpha: Math.random()*0.5+0.1 };
  }
...
function appendMessage(role, content, animate = false, imageDataUrl = null) {
  ...
  if (role === "assistant" && animate) {
    ...
    function typeChar() {
      ...
      } else { div.classList.remove("typing-cursor"); }
    }
    typeChar();
  } else {
...
formEl.addEventListener("submit", async (event) => {
  ...
  const imgToSend = attachedImageDataUrl;
  promptEl.value = "";
  attachedImageDataUrl = null;
  imagePreviewWrap.style.display = "none";
  imagePreview.src = "";

  appendMessage("user", text || "📎 Image", false, imgToSend);
