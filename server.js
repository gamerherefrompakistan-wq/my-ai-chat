...
async function callGemini(messages) {
  ...
  if (!candidate) throw new Error("Gemini ne koi jawab nahi diya");
  if (candidate.finishReason === "SAFETY") throw new Error("Message safety filter se block hua");

  const text = candidate?.content?.parts?.[0]?.text;
  if (!text) throw new Error("Gemini response mein text nahi mila");
  return text;
}
...
/* =========================
   VIDEO GENERATION — HuggingFace FREE
...
app.post("/api/generate-video", async (req, res) => {
  try {
    ...
    if (!hfToken) {
      return res.status(503).json({ error: "HF_TOKEN Railway pe set nahi hai" });
    }

    console.log("[Video] Generating with HuggingFace, prompt:", prompt);
    ...
    console.log("[Video] HF Status:", response.status);
    const contentType = response.headers.get("content-type") || "";

    // Model abhi load ho raha hai
    if (response.status === 503) {
      const errData = await response.json().catch(() => ({}));
      const wait = Math.ceil(errData?.estimated_time || 60);
      return res.status(503).json({
        error: `Model load ho raha hai, ${wait} seconds mein dobara try karo.`,
        retry_after: wait
      });
    }
    ...
    if (!response.ok) {
      ...
      console.error("[Video] Error:", errText.slice(0, 500));
      return res.status(502).json({ error: `HuggingFace error ${response.status}: ${errText.slice(0, 200)}` });
    }

    // Success — binary video data
    res.setHeader("Content-Type", "video/mp4");
    res.setHeader("Content-Disposition", "inline; filename=video.mp4");
    const buf = Buffer.from(await response.arrayBuffer());
    console.log("[Video] Done! Size:", buf.length, "bytes");
    return res.send(buf);
    ...
    if (err.name === "TimeoutError" || err.name === "AbortError") {
      return res.status(504).json({
        error: "Video generate hone mein zyada time laga. Dobara try karo — ab jaldi hoga."
      });
    }
